...
router.get('/', async (req, res) => {
  try {
    ...
    const cacheKey = `fconsumo_${familia || 'all'}`;
    ...
    let query = `
      ...
      LEFT JOIN prd_produtoclas pca ON pca.cd_produto = fc.cd_produtopa AND pca.cd_tipoclas = 111
    ...
    if (familia) {
      query += ` WHERE UPPER(cf.ds_classificacao) = UPPER($1)`;
      params.push(familia);
    }
    ...
    console.error('Erro ao buscar fconsumo:', error);
